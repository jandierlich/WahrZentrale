...
const KW_POI_TYPES = [
  { id: "viewpoint", label: "Aussichtspunkt", icon: "viewpoint", query: 'node["tourism"="viewpoint"]', group: "Natur & Ruhe" },
  { id: "park", label: "Park", icon: "tree", query: 'way["leisure"="park"]["access"!="private"]', group: "Natur & Ruhe" },
  { id: "beach", label: "Strand/Seeufer", icon: "beach", query: ['node["natural"="beach"]', 'way["natural"="beach"]'], group: "Natur & Ruhe" },
  { id: "naturereserve", label: "Naturschutzgebiet", icon: "forest", query: 'way["leisure"="nature_reserve"]', group: "Natur & Ruhe" },
  { id: "orchard", label: "Streuobstwiese", icon: "orchard", query: 'way["landuse"="orchard"]', group: "Natur & Ruhe" },
  { id: "cafe", label: "Café", icon: "coffee", query: 'node["amenity"="cafe"]', group: "Essen & Trinken" },
  { id: "icecream", label: "Eisdiele", icon: "icecream", query: 'node["amenity"="ice_cream"]', group: "Essen & Trinken" },
  { id: "restaurant", label: "Restaurant", icon: "restaurant", query: 'node["amenity"="restaurant"]', group: "Essen & Trinken" },
  { id: "bakery", label: "Bäckerei", icon: "bakery", query: 'node["shop"="bakery"]', group: "Essen & Trinken" },
  { id: "supermarket", label: "Supermarkt", icon: "cart", query: 'node["shop"="supermarket"]', group: "Einkaufen" },
  { id: "pharmacy", label: "Apotheke", icon: "pill", query: 'node["amenity"="pharmacy"]', group: "Einkaufen" },
  { id: "bank", label: "Bank", icon: "bank", query: 'node["amenity"="bank"]', group: "Einkaufen" },
  { id: "marketplace", label: "Wochenmarkt", icon: "basket", query: 'node["amenity"="marketplace"]', group: "Einkaufen" },
  { id: "farmshop", label: "Hofladen", icon: "flower", query: 'node["shop"="farm"]', group: "Einkaufen" },
  { id: "fuel", label: "Tankstelle", icon: "fuel", query: 'node["amenity"="fuel"]', group: "Mobilität" },
  { id: "parking", label: "Parkplatz", icon: "parking", query: 'node["amenity"="parking"]', group: "Mobilität" },
  { id: "station", label: "Bahnhof", icon: "train", query: 'node["railway"="station"]', group: "Mobilität" },
  { id: "transit", label: "Haltestelle", icon: "bus-stop", query: 'node["highway"="bus_stop"]', group: "Mobilität" },
  { id: "hospital", label: "Krankenhaus", icon: "hospital", query: 'node["amenity"="hospital"]', group: "Gesundheit" },
  { id: "attraction", label: "Sehenswürdigkeit", icon: "ferris-wheel", query: 'node["tourism"="attraction"]', group: "Freizeit & Kultur" },
  { id: "worship", label: "Kirche", icon: "church", query: 'node["amenity"="place_of_worship"]', group: "Freizeit & Kultur" },
  { id: "theatre", label: "Theater", icon: "masks", query: 'node["amenity"="theatre"]', group: "Freizeit & Kultur" },
  { id: "museum", label: "Museum", icon: "museum", query: 'node["tourism"="museum"]', group: "Freizeit & Kultur" },
  { id: "zoo", label: "Tierpark", icon: "zoo", query: 'way["tourism"="zoo"]', group: "Freizeit & Kultur" },
  { id: "themepark", label: "Freizeitpark", icon: "coaster", query: 'way["tourism"="theme_park"]', group: "Freizeit & Kultur" },
  { id: "hotel", label: "Hotel", icon: "hotel", query: 'node["tourism"="hotel"]', group: "Übernachtung" },
];
...
async function kwReverseGeocode(lat, lon){
  if (!await kwAskOnce("kw-consent-nominatim-reverse", "Automatische Ortsnamen-Ermittlung via OpenStreetMap Nominatim nutzen? Dein aktueller Standort (Koordinaten) wird dafür bei jeder Standortbestimmung an nominatim.openstreetmap.org gesendet, damit statt Koordinaten ein lesbarer Ortsname angezeigt werden kann.")) {
...
async function kwSearchLocationText(q){
  ...
  if (!await kwAskOnce("kw-consent-nominatim", "Externe Ortssuche via OpenStreetMap Nominatim nutzen? Der eingegebene Text wird dafür an nominatim.openstreetmap.org gesendet.")) return;
...
function kwUpdateAR(heading){
  ...
  projected.forEach(m => {
    if (m.kind === "kw-ar-poi" && typeof m.lat === "number") {
    ...
    kwState.pois.forEach(p => {
      ...
      div.innerHTML = `
        ...
        <div class="kw-poi-item-actions">
          <a class="kw-maps-link" href="${kwMapsUrl(p.lat, p.lon, p.name)}" target="_blank" rel="noopener" aria-label="In Karten-App öffnen"><svg class="wzi" aria-hidden="true"><use href="#wzi-map"></use></svg></a>
...
function kwRenderWaypoints(){
  ...
  kwState.waypoints.forEach(wp => {
    ...
    div.innerHTML = `
      ...
      <a class="kw-maps-link" href="${kwMapsUrl(wp.lat, wp.lon, wp.name)}" target="_blank" rel="noopener" aria-label="In Karten-App öffnen"><svg class="wzi" aria-hidden="true"><use href="#wzi-map"></use></svg></a>
